          ...
          Container(
            ...
            child: Consumer<ProductDetailsProvider>(
              builder: (ctx, details, child) {
                ...
                if (widget.product?.choiceOptions != null && details.variationIndex != null) {
                  for (int index = 0; index < widget.product!.choiceOptions!.length; index++) {
                    if (widget.product!.choiceOptions![index]?.options != null &&
                        details.variationIndex![index] <
                            widget.product!.choiceOptions![index]!.options!.length) {
                      variationList.add(
                      ...
                      // Handle the case where either choiceOptions or options is null
                      // or where variationIndex is out of bounds.
                      // You might want to log an error or handle it according to your use case.
                    }
                  }
                }
                ...
                if (widget.product != null && widget.product!.variation != null) {
                  variationType = variationType.replaceAll(' ', '');
                                      ...
                                      children: [
                                        ...
                                        Expanded(
                                          child: CustomButton(
                                            radius: 6,
                                            buttonText: getTranslated(
                                              (stock ?? 0) < (widget.product?.minimumOrderQty ?? 0) &&
                                                  widget.product?.productType == "physical"
                                                  ? 'out_of_stock'
                                                  : 'add_to_cart',
                                              context,
                                            ),
                                            onTap: (stock ?? 0) < (widget.product?.minimumOrderQty ?? 0) &&
                                                widget.product?.productType == "physical"
                                                ? null
                                                : () {
                                              if ((stock ?? 0) >= (widget.product?.minimumOrderQty ?? 0) ||
                                                  widget.product?.productType == "digital") {
