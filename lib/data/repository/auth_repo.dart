...
class AuthRepo {
  ...
  Future<ApiResponse> registration(RegisterModel register) async {
      ...
      FormData formData = FormData.fromMap({
        ...
        "image": register.image != null ? await MultipartFile.fromFile(register.image!.path) : null,
        "address": register.address,
        "jobAs_id": register.jobAs_id,
        "goverment_id": register.goverment_id,
        "city": register.city,
        "password": register.password,
        "f_name": register.fName,
    ...
    try {
      print('Registration Request: $register'); // Log the request data
      Response response = await dioClient!.post(
        AppConstants.registrationUri,
        data:formData ,
      );
      print('Registration Response: $response'); // Log the response data
      return ApiResponse.withSuccess(response);
    } catch (e) {
  ...
  Future<ApiResponse> updateToken() async {
    try {
      ...
      Response response = await dioClient!.post(AppConstants.tokenUri,
          data: {"cm_firebase_token": deviceToken},
          options: Options(headers: {
            'Authorization':
                'Bearer ${Provider.of<AuthProvider>(Get.context!, listen: false).authRepo?.getUserToken()}'
          }));
      return ApiResponse.withSuccess(response);
    } catch (e) {
      return ApiResponse.withError(ApiErrorHandler.getMessage(e));
