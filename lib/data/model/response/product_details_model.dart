...
class ProductDetailsModel {
  ...
  ProductDetailsModel.fromJson(Map<String, dynamic> json) {
    ...
    dynamic imagesValue = json['images'];

    if (imagesValue is String) {
      // If 'images' is a string, create a list with a single item
      _images = [imagesValue];
    } else if (imagesValue is List) {
      // If 'images' is a list, cast it to List<String>
      _images = List<String>.from(imagesValue);
    } else {
      // Handle other cases or provide a default value
      _images = []; // Default value or appropriate handling
    }
    if (json['color_image'] != null) {
      dynamic colorImageValue = json['color_image'];

      if (colorImageValue is String) {
        // If 'color_image' is a string, handle it accordingly
        // You might want to parse the string or perform other actions based on your requirements
      } else if (colorImageValue is List) {
        // If 'color_image' is a list, proceed with your existing logic
        _colorImage = <ColorImage>[];
        colorImageValue.forEach((v) {
    ...
    if (json['colors_formatted'] != null) {
      _colors = <Colors>[];
      for (var color in json['colors_formatted']) {
        _colors!.add(Colors.fromJson(color));
      }
    }
    ...
    if (json['attributes'] != null) {
      try {
        ...
        _attributes = []; // Default value or appropriate handling
      }
    } else {
      _attributes = []; // Default value if 'attributes' is null
    }

    if (json['choice_options'] != null) {
      dynamic choiceOptionsValue = json['choice_options'];

      if (choiceOptionsValue is String) {
        // If 'choice_options' is a string, handle it accordingly
        // You might want to parse the string or perform other actions based on your requirements
      } else if (choiceOptionsValue is List) {
        // If 'choice_options' is a list, proceed with your existing logic
        _choiceOptions = <ChoiceOptions>[];
        choiceOptionsValue.forEach((v) {
    ...
    if (json['variation'] != null) {
      dynamic variationValue = json['variation'];

      if (variationValue is String) {
        // If 'variation' is a string, handle it accordingly
        // You might want to parse the string or perform other actions based on your requirements
      } else if (variationValue is List) {
        // If 'variation' is a list, proceed with your existing logic
        _variation = <Variation>[];
        variationValue.forEach((v) {
...
// Assuming json['shipping_cost'] is a dynamic value from JSON parsing
    dynamic shippingCostValue = json['shipping_cost'];

// Check if the value is not null before calling toDouble()
    if (shippingCostValue != null) {
      _shippingCost = shippingCostValue.toDouble();
    } else {
      // Handle the case where 'shipping_cost' is null, assign a default value, or take appropriate action.
      _shippingCost = 0.0; // You can use any default value that makes sense for your application.
    }
